...
**Agent harness, human present**: when the CLI detects an agent harness with a human at the keyboard, `railway up` opens the browser and skips the confirm prompt — the agent invocation is treated as consent. A real human still has to complete OAuth in the browser.
...
1. **Preferred — background execution** (when the agent environment supports background commands and output polling):
