...
## Step 0: Set Agent Skill Metadata

Set `ide` to `agent-skill` and `version` to `0.0.0` for the survey URL. These are passed as hidden fields — the user never sees or enters them.
...
Store these values as `$IDE_TYPE` and `$PLUGIN_VERSION` for use in Step 2.
...
Replace `$IDE_TYPE` and `$PLUGIN_VERSION` with the values from Step 0. Example: `https://pulse.amazon/survey/MY0ZY7UA?ide=agent-skill&version=0.0.0`
